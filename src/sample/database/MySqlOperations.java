...
public class MySqlOperations {
    ...
    public ArrayList<Item> getAllPosts() throws SQLException {
        PreparedStatement pstm=con.prepareStatement("select * from item order by datePosted desc;");
        ResultSet r=pstm.executeQuery();
    ...
    public ArrayList<Item> getAllMyPosts() throws SQLException {
        PreparedStatement pstm=con.prepareStatement("select * from item where postedBy=? order by datePosted desc ;");
    ...
    public ArrayList<Requests> getMyRequests() throws SQLException {
        ...
        PreparedStatement pstm=con.prepareStatement("select * from requests where fromUser=? order by onDate desc");
        pstm.setInt(1,Params.userId);
        ResultSet r=pstm.executeQuery();
        while (r.next()){
    ...
    public ArrayList<Requests> getRequestsOnMyProducts() throws SQLException {
        ArrayList<Requests> requests=new ArrayList<>();
        PreparedStatement pstm=con.prepareStatement("select * from requests where toUser=? order by onDate desc;");
        pstm.setInt(1,Params.userId);
        ResultSet r=pstm.executeQuery();
        while (r.next()){
            Requests req=new Requests();
    ...
    public ArrayList<Rewards> getAllRewards() throws SQLException {
        PreparedStatement pstm=con.prepareStatement("select * from rewards where toUser=? order by dateRecieved desc;");
        pstm.setInt(1,Params.userId);
        ResultSet r=pstm.executeQuery();
